"""Script to write the expanded desc_extraction_provider.py"""
import os
...
CONDUCTORS_PATTERN = re.compile(r'\b(\d{1,2})/(\d)\b', re.I)
...
AMPERAGE_PATTERN = re.compile(r'(\d+(?:\.\d+)?)\s*(?:a(?:mps?)?|amp)', re.I)
WATTAGE_PATTERN = re.compile(r'(\d+)\s*(?:w(?:atts?)?)\b', re.I)
...
class DescriptionExtractionProvider(EvidenceProvider):
    ...
    def fetch_from_row(self, row: dict) -> dict:
        ...
        diameter = _extract_single(DIAMETER_PATTERN, desc)
        if diameter:
            facts["Diameter"] = (diameter, "in", ev)

        thickness = _extract_single(THICKNESS_PATTERN, desc)
        if thickness:
            facts["Thickness"] = (thickness, "in", ev)

        arbor = _extract_single(ARBOR_PATTERN, desc)
        if arbor:
            facts["Arbor Size"] = (arbor, "in", ev)

        rpm = _extract_single(RPM_PATTERN, desc)
        if rpm:
        ...
        profile = _extract_keyword_match(PROFILE_KEYWORDS, desc_lower)
        if profile:
            facts["Profile"] = (profile, None, ev)

        fitting = _extract_keyword_match(FITTING_TYPE_KEYWORDS, desc_lower)
        if fitting:
            facts["Fitting Type"] = (fitting, None, ev)

        conn = _extract_keyword_match(CONNECTION_KEYWORDS, desc_lower)
        if conn:
            facts["Connection Type"] = (conn, None, ev)

        pipe_size = _extract_single(PIPE_SIZE_PATTERN, desc)
        if pipe_size:
            facts["Pipe Size"] = (pipe_size, "in", ev)

        flow = _extract_single(FLOW_RATE_PATTERN, desc)
        if flow:
            facts["Flow Rate"] = (flow, "gpm", ev)

        pressure = _extract_single(MAX_PRESSURE_PATTERN, desc)
        if pressure:
            facts["Maximum Pressure"] = (pressure, "psi", ev)

        handles = _extract_single(NUM_HANDLES_PATTERN, desc)
        if handles:
            facts["Number of Handles"] = (handles, None, ev)

        voltage = _extract_single(VOLTAGE_PATTERN, desc)
        if voltage:
            facts["Voltage Rating"] = (voltage, "V", ev)

        amps = _extract_single(AMPERAGE_PATTERN, desc)
        if amps:
            facts["Amperage Rating"] = (amps, "A", ev)

        watts = _extract_single(WATTAGE_PATTERN, desc)
        if watts:
            facts["Wattage"] = (watts, "W", ev)

        wire_gauge = _extract_single(WIRE_GAUGE_PATTERN, desc)
        if wire_gauge:
            facts["Wire Gauge"] = (wire_gauge, "AWG", ev)

        cond_match = CONDUCTORS_PATTERN.search(desc)
        if cond_match:
            facts["Number of Conductors"] = (cond_match.group(1), None, ev)

        thread = _extract_single(THREAD_PATTERN, desc)
        if thread:
            facts["Thread Size"] = (thread, None, ev)

        head = _extract_single(HEAD_PATTERN, desc)
        if head:
            facts["Head Type"] = (head, None, ev)

        drive = _extract_single(DRIVE_PATTERN, desc)
        if drive:
            facts["Drive Type"] = (drive, None, ev)

        wood = _extract_single(WOOD_SPECIES_PATTERN, desc)
        if wood:
            facts["Wood Species"] = (wood, None, ev)

        grade = _extract_single(GRADE_PATTERN, desc)
        if grade:
            facts["Grade"] = (grade, None, ev)

        treatment = _extract_single(TREATMENT_PATTERN, desc)
        if treatment:
...
target = os.path.join(os.path.dirname(__file__), '..', 'files', 'desc_extraction_provider.py')
with open(target, 'w', encoding='utf-8') as f:
    f.write(content)
print(f"Written to {target}: {len(content)} bytes, {content.count(chr(10))} lines")
